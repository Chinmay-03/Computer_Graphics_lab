...
void bresenhamcircle::circle(int x, int y, int r) {
  limit = 0;
  xi = 0;
  yi = r;
  del = 2 * (1 - r);
...
void bresenhamcircle::mh() {
  xi = xi + 1;
  del = del + 2 * xi + 1;
}

void bresenhamcircle::mv() {
  yi = yi - 1;
  del = del - 2 * yi + 1;
}

void bresenhamcircle::md() {
  xi = xi + 1;
  yi = yi - 1;
  del = del + 2 * xi - 2 * yi + 2;
}
...
int main() {
  ...
  outtextxy(50,50,"SE AI-DS B 29 Chinmay Kotkar");
